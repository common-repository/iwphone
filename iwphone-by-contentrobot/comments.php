...
<?php if ('open' == $post-> comment_status) : ?>
	<div class="commentwrapper">
		<div id="commentsicon">
			<span id="commentsicon" class="icon-stack icon-3x">
		  		<i class="icon-circle icon-stack-base icon-3x"></i>
		  		<i class="icon-comments icon-light icon-3x"></i>
			</span>
		</div>
	...
	<?php if ($comments) : ?>
		...
		<?php foreach ($comments as $comment) : ?>
			...
			<li class="<?php echo $oddcomment; ?>" id="comment-<?php comment_ID() ?>">
				...
				<?php endif; ?>
				<br />
				<div class="commentmetadata"><?php comment_date('m.j.y') ?> @ <?php comment_time() ?> <?php edit_comment_link('Edit Comment','[ ',' ]'); ?></div>
				<?php comment_text() ?>
			</li>
...
<?php else : ?>
	...
	<?php else : ?>
		<p><label for="author">Name <small><span><?php if ($req) _e('(required)'); ?></span></small></label><input type="text" name="author" id="author" value="<?php echo $comment_author; ?>" size="22" tabindex="1" /></p>
		<p><label for="email">Mail <small>(will not be published) <span><?php if ($req) _e('(required)'); ?></span></small></label><input type="text" name="email" id="email" value="<?php echo $comment_author_email; ?>" size="22" tabindex="2" /></p>
